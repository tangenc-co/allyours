  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      ...
      if (!isDragging.current) {
        offset.current += (options.direction ?? -1) * timePassed * 0.00001 * AUTO_SCROLL_SPEED
    ...
    return () => {
      ...
      planes.current.forEach((plane) => {
        if (plane.material instanceof THREE.ShaderMaterial) {
          ...
          if (plane.material.uniforms.tex.value) (plane.material.uniforms.tex.value as THREE.Texture).dispose()
        }
